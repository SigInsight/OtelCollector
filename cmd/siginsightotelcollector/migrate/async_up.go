...
func registerAsyncUp(parentCmd *cobra.Command, logger *zap.Logger) {
	syncUpCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			up, err := newAsyncUp(config.Clickhouse.DSN, config.MigrateAsyncUp.Timeout, logger)
			if err != nil {
				return err
			}
...
func newAsyncUp(dsn string, timeout time.Duration, logger *zap.Logger) (*asyncUp, error) {
