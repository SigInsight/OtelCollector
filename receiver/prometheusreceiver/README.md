...
## Local scrape-only fork

This package is a local, scrape-only fork of the upstream OpenTelemetry
Collector Contrib `prometheusreceiver` v0.152.0. Its scrape engine is derived
from Prometheus v0.311.4-0.20260507094802-91c184a899b8.
...
## Source and upgrade contract

The receiver and OTLP conversion code originate from upstream
`prometheusreceiver` v0.152.0. The local scrape engine under
`internal/scrape` originates from Prometheus
v0.311.4-0.20260507094802-91c184a899b8 and is adapted to consume the local
scrape-only configuration model in `internal/scrapeconfig`.

When upgrading either upstream source, update the receiver, `scrapeconfig`, and
scrape engine as one compatibility unit. The upgrade must keep their focused
tests synchronized and rerun the tagged production dependency negative checks.
Those checks must continue to reject the full Prometheus config/API/remote
storage packages, unsupported discovery integrations, and their cloud
authentication SDKs from the compiled receiver and collector dependency
closures. This is a compiled-closure constraint; it does not claim that those
modules are absent from the repository's complete Go module graph.
...
This receiver runs Prometheus-compatible scraping for the supported subset
listed above. Configuration uses the familiar Prometheus `global` and
`scrape_configs` structure, but only the documented discovery integrations and
scrape fields are accepted.
...
- Exports metrics to an OTLP endpoint
...
- **Enable debug logs**: Enable debug-level logs in the collector to see detailed scrape and discovery errors:
...
1. **Limit Target Count**
  ...
  - Manually shard targets or jobs across collector replicas
...
2. **Inspect Collector Logs and Metrics**
  - Use debug logs to inspect target discovery and scrape failures
  - Monitor the receiver scrape metrics and collector pipeline metrics
