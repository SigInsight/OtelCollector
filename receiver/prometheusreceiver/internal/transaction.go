...
import (
	...
	mdata "github.com/SigInsight/OtelCollector/receiver/prometheusreceiver/internal/metadata"
...
// scrape appender paths.
func (t *transaction) addSampleDatapoint(rKey resourceKey, ls labels.Labels, metricName string, atMs int64, val float64, stMs int64) storage.SeriesRef {
	...
	return storage.SeriesRef(ls.Hash())
}
...
// It is shared by scrape appender paths.
func (t *transaction) addHistogramDatapoint(rKey resourceKey, ls labels.Labels, metricName string, atMs int64, h *histogram.Histogram, fh *histogram.FloatHistogram, schema int32, stMs int64) storage.SeriesRef {
	parsedScope, attrs := getScopeID(ls)
	t.addScopeAttributesFromLabels(rKey, parsedScope, attrs)
...
func (t *transaction) Append(_ storage.SeriesRef, ls labels.Labels, stMs, atMs int64, val float64, h *histogram.Histogram, fh *histogram.FloatHistogram, opts storage.AppendV2Options) (storage.SeriesRef, error) {
	...
	if isHistogram {
		...
		sRef = t.addHistogramDatapoint(*rKey, ls, metricName, atMs, h, fh, schema, stMs)
	} else {
		t.addingNativeHistogram = false
		t.addingNHCB = false

		sRef = t.addSampleDatapoint(*rKey, ls, metricName, atMs, val, stMs)
	}
