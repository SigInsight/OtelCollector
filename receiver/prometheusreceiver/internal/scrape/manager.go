...
func (m *Manager) ApplyConfig(cfg *scrapeconfig.Config) error {
	...
	for poolName, pool := range m.scrapePools {
		...
		go func(name string, sp *scrapePool, cfg *scrapeconfig.ScrapeConfig, ok bool) {
			defer func() {
				wg.Done()
				<-canReload
			}()
...
func (m *Manager) ScrapePoolConfig(scrapePool string) (*scrapeconfig.ScrapeConfig, error) {
	m.mtxScrape.Lock()
	defer m.mtxScrape.Unlock()
